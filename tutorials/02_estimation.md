...
by method and treatment value. We walk **three graphs of increasing difficulty**:

1. a **back-door** graph — all three estimators (OM / IPW / DML);
2. a **multi-treatment sequential (mSBD)** graph — effects of a joint `do(X1, X2)`;
3. the **Napkin** graph — a c-component effect, where estimation runs in
   outcome-model-only mode.
...
- **IPW** — reweight observed outcomes with balancing weights so the treated and
  untreated covariate distributions match.
- **DML** — combine both into a doubly-robust estimate (consistent if *either* the
  outcome model or the weights are right), cross-fitted to remove overfitting bias.
...
conditionals — no adjustment set exists. The dispatcher routes such graphs to the
c-component (Tian) estimator, which currently runs in **outcome-model-only mode**:
the returned dictionary has just the plug-in `OM` entry, and the IPW/DML composition
for c-components is not implemented (see the README's *Status and limitations*).
...
The plug-in estimate approximates the ratio estimand directly from the data.
The c-component path is the **experimental** part of CausalPy: expect it on simple
discrete graphs like this one, and expect a clear `NotImplementedError` (rather than a
wrong number) where the decomposition or the estimator composition is unsupported.
...
| Tian / c-component (e.g. Napkin) | `est_general.estimate_*` | OM only (experimental) |
