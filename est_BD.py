...
if __name__ == "__main__":
	[scm, X, Y] = random_generator.Random_SCM_Generator(num_observables = 10, num_unobservables = 2, num_treatments = 3, num_outcomes = 1, 
																			condition_ID = True, condition_BD = True, condition_mSBD = True, condition_FD = False, condition_Tian = True, condition_gTian = True)
	G = scm.graph
	...
	if len(X) == 1:
		...
		for x_val in X_values_combinations:
			...
			# Initialization 
			causal_effect_estimation[x_val] = 0
		...
		for _, x_val in X_values_combinations.iterrows():
			...
			# Initialization 
			causal_effect_estimation[tuple(x_val)] = 0
	...
	if len(Z) == 0:
		...
		if len(X) == 1: 
			for x_val in X_values_combinations:
				causal_effect_estimation[x_val] /= L
		else:
			for _, x_val in X_values_combinations.iterrows():
