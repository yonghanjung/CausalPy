...
def random_SCM_generator(num_observables, num_unobservables, num_treatments, num_outcomes, **kwargs):
    """
    Generates a random graph and immediately builds a Structural Causal Model (SCM) from it.

    This is a convenience wrapper around random_graph_generator. It accepts the exact
    same arguments and returns an initialized SCM object instead of just the graph dictionary.

    Returns:
        A list containing [scm, X, Y] if a graph is found, otherwise None.
    """
    # 1. Find a graph dictionary using the existing function
    result = random_graph_generator(
        num_observables=num_observables,
        num_unobservables=num_unobservables,
        num_treatments=num_treatments,
    ...
    # 2. If a graph was found, build the SCM
    if result:
        graph_dict, _, X, Y = result
        scm = SCM.StructuralCausalModel()
        
        # Determine if the SCM should use discrete variables
        discrete = kwargs.get('discrete', True)
        scm.build_from_graph(graph_dict, discrete=discrete)
        
        return [scm, X, Y]
    
    # Return None if no graph was found
    return None

def find_SCM_by_search(max_observables, max_unobservables, num_treatments, num_outcomes, **kwargs):
    """
    Searches for a graph that meets specific criteria and builds a Structural Causal Model (SCM) from it.

    This is a convenience wrapper around find_graph_by_search. It accepts the exact
    same arguments and returns an initialized SCM object instead of just the graph dictionary.

    Returns:
        A list containing [scm, X, Y] if a matching graph is found, otherwise None.
    """
    # 1. Find a graph that meets all the criteria
    result = find_graph_by_search(
        max_observables=max_observables,
        max_unobservables=max_unobservables,
        num_treatments=num_treatments,
        num_outcomes=num_outcomes,
        **kwargs
    )
